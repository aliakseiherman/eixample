...
export class TeamServiceProxy {
    ...
    getAll(): Observable<TeamDto[] | null> {
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return <Observable<TeamDto[] | null>><any>_observableThrow(e);
                }
            } else
                return <Observable<TeamDto[] | null>><any>_observableThrow(response_);
        }));
    }

    protected processGetAll(response: HttpResponseBase): Observable<TeamDto[] | null> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        return _observableOf<TeamDto[] | null>(<any>null);
    }
}
...
export class TeamDto extends EntityDtoOfInt64 implements ITeamDto {
    name: string | undefined;
...
export interface ITeamDto extends IEntityDtoOfInt64 {
    name: string | undefined;
    description: string | undefined;
}
